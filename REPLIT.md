...
Render Secret Files are mounted under `/run/secrets/`. To configure one:
...
4. Add the environment variable `YT_DLP_COOKIEFILE` with this exact value: `/run/secrets/www.youtube.com_cookies.txt`.
5. Restart or redeploy the service. Secret Files are mounted after restart.
6. In the Render Shell, verify only the file metadata with `ls -l /run/secrets/`; never print the file contents.
